...
describe("ServerCard group controls", () => {
  it("does not render any group menu or inline selector", () => {
    ...
    // 卡片不再有分组选择行、溢出菜单触发器或 FolderInput 图标。
    expect(markup).not.toContain("Move to group");
    expect(markup).not.toContain("FolderInput");
    expect(markup).not.toContain("Move My Server to group");
    expect(markup).not.toContain('role="menu"');
  });

  it("keeps only start/stop and remove buttons in the control area", () => {
    const markup = renderCard(baseServer, "full");
    expect(markup).toContain("Stop server");
    expect(markup).toContain("Remove My Server");
    // 控制区只有这两个按钮(Start 在停止态出现)。
    const stoppedMarkup = renderCard({ ...baseServer, status: "stopped" }, "full");
    expect(stoppedMarkup).toContain("Start server");
    expect(stoppedMarkup).toContain("Remove My Server");
  });
});
